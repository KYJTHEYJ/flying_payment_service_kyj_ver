...
public class PaymentService {
    ...
    // TODO 결제 적용시 주문 테이블에도 수정을 가해야하나 싶음
    @Transactional
    public CreatePaymentResponse createPayment(CreatePaymentRequest request) {
        ...
        if(request.getPointsToUse() != null && request.getPointsToUse() > 0) {
            order.updateUsePoint(request.getPointsToUse());
            order.updateActualPrice(order.getTotalOrderPrice() - request.getPointsToUse());
            order.updateSavePoint((int) Math.floor((order.getTotalOrderPrice() - request.getPointsToUse()) * order.getMember().getGrade().getPointRate() * 0.01));
        }
    ...
    public ConfirmPaymentResponse confirmPayment(String paymentId, String email) {
        ...
        List<ProductOrder> productOrderList = productOrderRepository.findByOrder(payment.getOrder());
        ...
        member.addPoint(payment.getOrder().getSavePoint());
        MemberPointLog savedSavePointLog = MemberPointLog.register(payment.getOrder().getOrderNo(), payment.getOrder().getSavePoint(), MemberPointLogStatus.SAVE, member);
        memberPointLogRepository.save(savedSavePointLog);

        // 고객 포인트 소모, 포인트 소모 이력 생성
        if (payment.getOrder().getUsePoint() > 0) {
            member.minusPoint(payment.getOrder().getUsePoint());
            MemberPointLog savedUsePointLog = MemberPointLog.register(payment.getOrder().getOrderNo(), payment.getOrder().getUsePoint(), MemberPointLogStatus.USE, member);
            memberPointLogRepository.save(savedUsePointLog);
        }

        return new ConfirmPaymentResponse(true, payment.getOrder().getOrderId().toString(), PaymentStatus.COMPLETE.name());
    }
}
